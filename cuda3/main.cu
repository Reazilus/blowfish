...
int main(int argc, char *argv[])
{
	...
	cudaCheck(cudaMemcpyToSymbol(ctx, context, sizeof(blowfish_context_t)));
	...
	cudaBlowfishDecryptPtr<<<gridsize, blocksize>>>(filegpu, numblocks);
	...
	cudaMemcpy(filegpu, file, filesize, cudaMemcpyHostToDevice);
	cudaBlowfishEncryptPtr<<<gridsize, blocksize>>>(filegpu, numblocks);
	cudaMemcpy(file, filegpu, filesize, cudaMemcpyDeviceToHost);	
